...
export default async function handler(req, res) {
  ...
  try {
    ...
    for (const d of due.rows) {
      ...
      if (!snaps.length) { log.push({ draw: d.id, action: "skipped", why: "no snapshot yet" }); continue; }
      const snap = snaps[0];
      ...
      const eligible = await filterEligible(entriesQ.rows, snap.id);
      ...
      await sql`
        ...
        snapshot_id = ${snap.id}, next_index = ${nextIndex} WHERE id = ${d.id}`;
    ...
    for (const w of expired.rows) {
      ...
      const eligible = await filterEligible(entriesQ.rows, w.snapshot_id);
...
// Eligibility inside a draw: wallet must appear in the last K consecutive snapshots.
async function filterEligible(entries, latestSnapshotId) {
  const snaps = await lastTakenSnapshots(CONFIG.K_CONSECUTIVE);
  if (snaps.length < CONFIG.K_CONSECUTIVE) return []; // warming up: holders not yet eligible
