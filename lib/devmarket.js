...
export async function devmarket(req, res) {
  ...
  try {
    ...
    if (b.action === "seller-apply") {
      ...
      const r = await sql`
        INSERT INTO dev_sellers(wallet, studio, contact, website, steam_appids, payout_currency)
        VALUES (${wallet}, ${b.studio || null}, ${b.contact || null}, ${b.website || null},
                ${b.steam_appids || null}, ${payout})
    ...
    if (b.action === "seller-status") {
      ...
      const prods = await sql`
        SELECT p.id, p.title, p.appid, p.image, p.edition, p.region, p.price_cents,
               p.steam_price_cents, p.active, p.created_at,
    ...
    if (b.action === "seller-queue") {
      ...
      const rows = await sql`
        SELECT wallet, studio, contact, website, steam_appids, payout_currency, status,
    ...
    if (b.action === "product-create") {
      ...
      const r = await sql`
        INSERT INTO dev_products(seller_id, title, appid, image, edition, region, price_cents, steam_price_cents)
        VALUES (${seller.id}, ${title}, ${b.appid || null}, ${b.image || null}, ${b.edition || null},
                ${b.region || "GLOBAL"}, ${cents}, ${steamCents})
        RETURNING id, title, price_cents, region, active`;
      return res.status(200).json({ ok: true, product: r.rows[0] });
    }

    // ---- SELLER: update a product (price / active) ------------------------
