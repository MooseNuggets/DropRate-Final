...
async function releaseOrder(orderId) {
  ...
  try {
    const sig = await sendTreasuryTransfer(dest, net);
    ...
    await sql`INSERT INTO dev_payouts(order_id, seller_id, gross_raw, fee_raw, net_raw, payout_currency, rail, external_ref, status, sent_at)
              VALUES (${o.id}, ${o.seller_id}, ${o.pay_amount_raw}, ${feeRaw}, ${net}, 'DROP', 'treasury', ${sig}, 'sent', now())`;
  ...
  try {
    ...
    if (b.action === "profile-set") {
      ...
      const r = await sql`
        ...
        WHERE wallet = ${wallet}
        RETURNING wallet, studio, contact, website, socials, steam_appids, avatar, bio`;
    ...
    if (b.action === "seller-edit") {
      ...
      const r = await sql`
        UPDATE dev_sellers SET
          ...
          standing = ${standing}, updated_at = now()
    ...
    if (b.action === "product-view") {
      const pid = Number(b.product_id);
      const dropUsd = await currentDropUsd().catch(() => null);
      const r = await sql`
        SELECT p.id, p.title, p.appid, p.image, p.region, p.price_cents, p.steam_price_cents,
               ...
               s.wallet AS seller_wallet, s.studio, s.standing, s.avatar, s.bio, s.website, s.socials,
               count(k.id) FILTER (WHERE k.status = 'available') AS available
        FROM dev_products p
        ...
        GROUP BY p.id, s.wallet, s.studio, s.standing, s.avatar, s.bio, s.website, s.socials`;
      const p = r.rows[0];
      if (!p) return res.status(404).json({ error: "game not found" });
      const usd = p.price_cents / 100;
      return res.status(200).json({
        ok: true, dropUsd, buyer_drop_discount_bps: BUYER_DROP_DISCOUNT_BPS,
        product: {
          ...
          drop_est_discounted: dropUsd ? Math.round((usd * (1 - BUYER_DROP_DISCOUNT_BPS / 10000)) / dropUsd) : null,
        },
        dev: {
    ...
    if (b.action === "market") {
      ...
      const rows = await sql`
        SELECT p.id, p.title, p.appid, p.image, p.edition, p.region, p.price_cents, p.steam_price_cents,
               s.wallet AS seller_wallet, s.studio, s.standing,
               count(k.id) FILTER (WHERE k.status = 'available') AS available
        FROM dev_products p
        JOIN dev_sellers s ON s.id = p.seller_id
        LEFT JOIN dev_key_inventory k ON k.product_id = p.id
        WHERE p.active = true AND s.status = 'approved'
        GROUP BY p.id, s.wallet, s.studio, s.standing
        HAVING count(k.id) FILTER (WHERE k.status = 'available') > 0
        ORDER BY p.created_at DESC
        LIMIT 200`;
      ...
      const listings = rows.rows.map((r) => {
        ...
        const dropWhole = dropUsd ? Math.round(usd / dropUsd) : null;
        const dropWholeDisc = dropUsd ? Math.round((usd * (1 - discBps / 10000)) / dropUsd) : null;
        return {
          ...
          drop_est: dropWhole, drop_est_discounted: dropWholeDisc,
        };
      });
      return res.status(200).json({
        ok: true, dropUsd, decimals: DECIMALS,
        buyer_drop_discount_bps: discBps,
        fee_bps: FEE_BPS,
        listings,
      });
    }
    ...
    if (b.action === "buy-open") {
      ...
      const pid = Number(b.product_id);
      const pr = await sql`
        SELECT p.*, s.status AS seller_status FROM dev_products p
      ...
      const dropUsd = await currentDropUsd().catch(() => null);
      if (!dropUsd) return res.status(503).json({ error: "price feed unavailable — try again" });
      const usd = product.price_cents / 100;
      // NOTE: buyer pays the full $DROP price in this phase. The buyer $DROP discount
      // only becomes meaningful once USDC/SOL exist to steer away from — it activates then.
      const grossRaw = BigInt(Math.round((usd / dropUsd) * Math.pow(10, DECIMALS)));
      const feeBps = feeBpsFor("DROP");
      ...
      const ord = await sql`
        ...
        VALUES (${pid}, ${product.seller_id}, ${keyId}, ${b.buyer}, 'DROP', 'DROP', ${product.price_cents},
      ...
      return res.status(200).json({
        ...
        pay_amount_raw: grossRaw.toString(), decimals: DECIMALS, expires_at: expires, title: product.title,
      });
    }
    ...
    if (b.action === "buy-buildpay") {
      ...
      const built = await buildDirectPaymentTx(b.buyer || o.buyer, o.reference, requireTreasury(), String(o.pay_amount_raw));
      return res.status(200).json({ ok: true, ...built, reference: o.reference });
    }
    ...
    if (b.action === "buy-confirm") {
      ...
      if (o.status !== "created") return res.status(409).json({ error: `order ${o.status}` });
      const found = await findPaymentByReference(o.reference);
    ...
    if (b.action === "dispute-resolve") {
      ...
      if (b.decision === "refund") {
        ...
        try { sig = await sendTreasuryTransfer(o.buyer, String(o.pay_amount_raw)); }
