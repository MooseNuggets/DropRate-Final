...
// sendTreasuryTransfer stays only to settle orders opened in $DROP before it left
// the payment path — legacy payouts and refunds. Nothing new is priced in it.
...
/* Keys are bought in SOL or USDC. Not $DROP.

   $DROP is deliberately NOT a payment option here and earns no discount — the
   token lives in the raffles and the crates. A game purchase stays a clean
   transaction in a stable-ish asset, and a developer is paid in exactly what the
   buyer paid, so the treasury never carries FX risk between the two. */
const CCY = ["USDC", "SOL"];
function ccyDecimals(cur) { return cur === "SOL" ? SOL_DECIMALS : USDC_DECIMALS; }
...
// A developer is paid in the currency the buyer used. Their accepted_currencies
// list is therefore also their payout preference — there is no separate
// payout_currency to get out of step with it.
const PAYOUT_CCYS = CCY;
const DEFAULT_ACCEPTED = ["USDC", "SOL"];
...
/* One flat platform fee on every key sale, whichever currency it's paid in.
   5% — a sixth of Steam. A single number on purpose: two rates is how a store
   ends up promising devs one figure and paying them another. */
const FEE_BPS = Number(process.env.DEV_FEE_BPS || 500);
...
async function notifyNewApplication(s) {
  ...
  try {
    const text = [
      ...
      `Accepts:       ${(s.accepted_currencies || DEFAULT_ACCEPTED).join(", ")}`, ``,
...
// ---- schema (idempotent) ----------------------------------------------------
//
// Once per warm instance, not once per request — see the note in nativemarket.js.
// ~20 DDL statements ran on every single marketplace call before this guard.
// Exported because nativemarket.js's schema references dev_sellers and has to be
// able to guarantee these tables exist first — see the note there.
let devMigration = null;
export function migrate() {
  if (!devMigration) {
    devMigration = runMigrate().catch((e) => { devMigration = null; throw e; });
  }
  return devMigration;
}
...
async function runMigrate() {
  await sql`CREATE TABLE IF NOT EXISTS dev_sellers(
    ...
    socials text,
    steam_appids text,
    payout_currency text NOT NULL DEFAULT 'USDC',  -- legacy; payouts now follow pay_currency
  ...
  await sql`ALTER TABLE dev_sellers ADD COLUMN IF NOT EXISTS accepted_currencies jsonb NOT NULL DEFAULT '["USDC","SOL"]'::jsonb`;
  /* $DROP left the payment path. Strip it from every existing seller's accepted
     list in one idempotent pass, and anyone whose list would be emptied by that
     falls back to the default rather than to "accepts nothing" — a seller who
     only ever ticked $DROP should still be sellable, not silently delisted. */
  await sql`ALTER TABLE dev_sellers ALTER COLUMN accepted_currencies SET DEFAULT '["USDC","SOL"]'::jsonb`;
  await sql`UPDATE dev_sellers SET accepted_currencies = (
      SELECT COALESCE(jsonb_agg(c), '[]'::jsonb) FROM jsonb_array_elements_text(accepted_currencies) AS c WHERE c IN ('USDC','SOL'))
    WHERE accepted_currencies ? 'DROP'`;
  await sql`UPDATE dev_sellers SET accepted_currencies = '["USDC","SOL"]'::jsonb
    WHERE accepted_currencies = '[]'::jsonb OR accepted_currencies IS NULL`;
  await sql`ALTER TABLE dev_sellers ALTER COLUMN payout_currency SET DEFAULT 'USDC'`;
  await sql`UPDATE dev_sellers SET payout_currency = 'USDC' WHERE payout_currency = 'DROP'`;
  ...
  await sql`CREATE TABLE IF NOT EXISTS dev_orders(
    ...
    buyer text NOT NULL,
    pay_currency text NOT NULL,                 -- USDC | SOL  (DROP on legacy rows only)
...
async function releaseOrder(orderId) {
  ...
  /* Paid out in the currency the buyer paid — pay_currency, not a separate
     seller preference. That is what makes the treasury FX-neutral: it never
     has to hold one asset to pay out another. Orders opened before this change
     carry a payout_currency of DROP; those are legacy and settle in what was
     actually received. */
  const cur = CCY.includes(o.pay_currency) ? o.pay_currency : (o.payout_currency === "DROP" ? "DROP" : "USDC");
  try {
...
const feeBpsFor = () => FEE_BPS;

export async function devmarket(req, res) {
  if (req.method !== "POST") return res.status(405).json({ error: "POST only" });
  try {
    const b0 = req.body ?? {};
    // Native-game / launcher actions live in a separate module but ride this same
    // ns:"devmarket" dispatch (so api/crate.js needs no change). They reuse the
    // shared dev_sellers identity but their own product tables.
    if (typeof b0.action === "string" && b0.action.startsWith("native-")) {
      const { nativemarket } = await import("./nativemarket.js");
      return await nativemarket(req, res);
    }
    // Game SDK (achievements / saves / leaderboards) for our own pages.
    if (typeof b0.action === "string" && b0.action.startsWith("sdk-")) {
      const { gamesdk } = await import("./gamesdk.js");
      return await gamesdk(req, res, b0.action, b0);
    }
    await migrate();
    const b = req.body ?? {};
    ...
    if (b.action === "seller-apply") {
      ...
      // What they'll accept — and therefore be paid in. Defaults to both.
      const accepted = cleanAccepted(b.accepted_currencies) || DEFAULT_ACCEPTED;
      const payout = accepted[0];
      const r = await sql`
        INSERT INTO dev_sellers(wallet, studio, contact, website, socials, steam_appids, payout_currency, accepted_currencies)
        VALUES (${wallet}, ${b.studio || null}, ${b.contact || null}, ${b.website || null},
                ${b.socials || null}, ${b.steam_appids || null}, ${payout}, ${JSON.stringify(accepted)}::jsonb)
      ...
      if (r.rows[0] && r.rows[0].status === "pending") {
        await notifyNewApplication({
          ...
          website: b.website, steam_appids: b.steam_appids, accepted_currencies: accepted,
        });
      }
    ...
    if (b.action === "seller-status") {
      ...
      return res.status(200).json({
        ...
        seller: {
          ...
          accepted_currencies: seller.accepted_currencies || DEFAULT_ACCEPTED,
        },
        products: prods.rows,
      });
    }
    ...
    if (b.action === "profile-set") {
      ...
      const acc = cleanAccepted(b.accepted_currencies) || seller.accepted_currencies || DEFAULT_ACCEPTED;
      const r = await sql`
    ...
    if (b.action === "seller-edit") {
      ...
      const payout = PAYOUT_CCYS.includes(b.payout_currency) ? b.payout_currency : (acc ? acc[0] : "USDC");
      const r = await sql`
        UPDATE dev_sellers SET
          studio = ${b.studio || null}, contact = ${b.contact || null},
          website = ${b.website || null}, socials = ${b.socials || null},
    ...
    if (b.action === "product-view") {
      ...
      return res.status(200).json({
        ok: true, solUsd, fee_bps: FEE_BPS,
        product: {
          ...
          accepted_currencies: (p.accepted_currencies || DEFAULT_ACCEPTED).filter((c) => CCY.includes(c)),
    ...
    // Returns the USD price plus a live SOL estimate. No $DROP pricing here —
    // keys are bought in USDC or SOL, and there is no discount for anything.
    if (b.action === "market") {
      const solUsd = await currentSolUsd().catch(() => null);
      const rows = await sql`
      ...
      const listings = rows.rows.map((r) => {
        ...
        return {
          ...
          accepted_currencies: (r.accepted_currencies || DEFAULT_ACCEPTED).filter((c) => CCY.includes(c)),
          usdc_amount: Number(usd.toFixed(2)),
          sol_est: solUsd ? Number((usd / solUsd).toFixed(4)) : null,
        };
      });
      return res.status(200).json({ ok: true, solUsd, fee_bps: FEE_BPS, listings });
    }

    // ======================================================================
    // ESCROW ORDER ENGINE. Buyer pays USDC or SOL to the treasury; the key is
    // delivered on payment; the dev's payout is HELD for the dispute window and
    // released (net of the flat 5%) in the same currency on buyer-confirm or
    // window-expiry. The treasury never converts between assets.
    // ======================================================================
    ...
    if (b.action === "buy-open") {
      ...
      if (!CCY.includes(b.pay_currency)) {
        return res.status(400).json({ error: "keys are paid for in USDC or SOL" });
      }
      ...
      const accepted = (product.accepted_currencies || DEFAULT_ACCEPTED).filter((c) => CCY.includes(c));
    ...
    if (b.action === "buy-buildpay") {
      ...
      if (!CCY.includes(o.pay_currency)) return res.status(409).json({ error: "this order was opened in a currency no longer accepted — start over" });
      const built = await buildDirectPaymentMulti(payer, o.reference, requireTreasury(), String(o.pay_amount_raw), o.pay_currency);
    ...
    if (b.action === "buy-confirm") {
      ...
      if (o.status !== "created") return res.status(409).json({ error: `order ${o.status}` });
      if (!CCY.includes(o.pay_currency)) return res.status(409).json({ error: "this order was opened in a currency no longer accepted" });
    ...
    if (b.action === "dispute-resolve") {
      ...
      if (b.decision === "refund") {
        ...
        // Refunded in exactly what was paid. "DROP" only ever matches a legacy order.
        const rc = o.pay_currency || "USDC";
