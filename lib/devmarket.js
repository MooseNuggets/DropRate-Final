...
const FEE_BPS = {
  ...
  USDC: Number(process.env.DEV_FEE_BPS_USDC || 700), // 7%
  SOL:  Number(process.env.DEV_FEE_BPS_SOL  || 700), // 7%
};
// Buyer-side incentive: paying in $DROP shaves this off the price. It is SELF-FUNDING,
// not a platform subsidy: the discount is sized to the fee gap (USDC/SOL fee − $DROP
// fee = 5.5%), so a dev nets ~the same (a hair more) on a $DROP sale as on a USDC/SOL
// sale. Buyer wins by paying in $DROP; dev is neutral-to-better; treasury never dips in.
