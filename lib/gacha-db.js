...
let gachaMigration = null;
export function migrateGacha() {
  if (!gachaMigration) {
    gachaMigration = runMigrateGacha().catch((e) => { gachaMigration = null; throw e; });
  }
  return gachaMigration;
}

async function runMigrateGacha() {
  ...
  // Multi-currency crates: which rail the pull was paid on. paid_raw /
  // amount_quoted_raw / refund_raw are ALL in base units of pay_currency
  // (DROP → DECIMALS, USDC → 6, SOL → 9). Pre-existing rows default to DROP.
  await sql`ALTER TABLE pulls ADD COLUMN IF NOT EXISTS pay_currency text NOT NULL DEFAULT 'DROP'`;
  await sql`ALTER TABLE pulls ADD COLUMN IF NOT EXISTS pay_decimals int`;
  await sql`ALTER TABLE pulls ADD COLUMN IF NOT EXISTS discount_bps int NOT NULL DEFAULT 0`;
  ...
  // Which rail the settlement row is denominated in (matches pulls.pay_currency).
  await sql`ALTER TABLE settlements ADD COLUMN IF NOT EXISTS currency text NOT NULL DEFAULT 'DROP'`;
}
