...
export async function devicepair(req, res, action, b) {
  ...
  /* ---- LAUNCHER: an SDK ticket for a game it is about to launch ----------
     The launcher already proved this device belongs to the wallet; ownership
     is re-checked here so a copy sold since the last library refresh gets no
     ticket. The ticket goes to the game as DROPRATE_TICKET in its environment. */
  if (action === "native-device-ticket") {
    const d = await deviceFor(b.device_token);
    const pid = Number(b.product_id);
    if (!pid) httpErr(400, "product_id required");
    const { walletOwnsCopy } = await import("./nativebuy.js");
    const o = await walletOwnsCopy(pid, d.wallet);
    if (!o.owns) httpErr(403, "this wallet no longer owns a copy of that game");
    const { issueTicket } = await import("./gamesdk.js");
    return res.status(200).json({ ok: true, ...issueTicket({ wallet: d.wallet, productId: pid, source: "launcher" }) });
  }
