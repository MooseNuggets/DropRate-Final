...
export function roundAt(unixSeconds) {
  ...
  return Math.floor((unixSeconds - DRAND.GENESIS) / DRAND.PERIOD) + 2; // strictly future round
}
...
// Fetch a round from multiple relays and require agreement
export async function fetchRandomness(round) {
  const results = [];
  for (const base of DRAND.RELAYS) {
    try {
      const res = await fetch(`${base}/${DRAND.CHAIN}/public/${round}`, { signal: AbortSignal.timeout(8000) });
      if (!res.ok) continue;
...
// pool: [{wallet, tickets}] — pre-sorted canonically by caller.
export function selectWinners(seedHex, drawId, pool, nWinners, excludeWallets = new Set(), startIndex = 0) {
  ...
  while (winners.length < nWinners && entries.length > 0) {
    ...
    entries.splice(pick, 1); // without replacement
    idx++;
  }
