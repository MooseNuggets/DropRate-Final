...
//
// Reward: once the streak reaches BONUS_START_DAYS, the wallet earns bonus raffle
// entries on top of its balance tickets, and the accrual RATE scales with tier —
// every STEP_DAYS of clean holding adds that tier's rate:
//   Bronze +1 · Silver +2 · Gold +3 · Diamond +5  (every other day)
// The total is the sum of those per-step accruals over the whole streak, so both
// holding longer AND climbing tiers grow the bonus. Uncapped unless STREAK.CAP set.

export const STREAK = {
  BONUS_START_DAYS: 10,   // clean-hold days before the first bonus entry (= Bronze)
  STEP_DAYS: 2,           // bonus accrues once per this many days ("every other day")
  CAP: 300,               // max total loyalty bonus (fully maxed at ~5.8mo of clean holding)
};

const DAY_MS = 86400000;
const toBig = (x) => BigInt(String(x).split(".")[0]);

// Bonus entries for a given clean-hold duration (in days). Walks the streak in
// STEP_DAYS increments and adds the tier rate in effect at each step, so a wallet
// that tiers up mid-streak earns the higher rate only from that point forward.
export function bonusFor(holdDays) {
  if (holdDays < STREAK.BONUS_START_DAYS) return 0;
  let b = 0, steps = 0;
  for (let d = STREAK.BONUS_START_DAYS; d <= holdDays && steps < 200000; d += STREAK.STEP_DAYS, steps++) {
    b += rateForDays(d);
  }
...
// `rate` = bonus raffle entries earned per STEP_DAYS of clean holding while in
// that tier — the amount scales as a wallet climbs. Single source of truth for
// both the display badge and the bonus accrual.
export const TIERS = [
  { key: "paper",   name: "Paper",   min: 0,   color: "#9AA6C7", rate: 0 },
  { key: "bronze",  name: "Bronze",  min: 10,  color: "#CD7F32", rate: 1 },
  { key: "silver",  name: "Silver",  min: 25,  color: "#C0C7D4", rate: 2 },
  { key: "gold",    name: "Gold",    min: 50,  color: "#F6AC29", rate: 3 },
  { key: "diamond", name: "Diamond", min: 100, color: "#5AD8F0", rate: 5 },
];

// The per-step bonus rate for a clean-hold duration (its current tier's rate).
export function rateForDays(holdDays) {
  const d = Number(holdDays) || 0;
  let rate = 0;
  for (let i = 0; i < TIERS.length; i++) if (d >= TIERS[i].min) rate = TIERS[i].rate;
  return rate;
}
...
export function tierFor(holdDays) {
  ...
  return {
    ...
    color: cur.color,
    minDays: cur.min,
    rate: cur.rate,                 // bonus entries earned per step at this tier
    nextName: nx ? nx.name : null,
    nextAtDays: nx ? nx.min : null,
    nextRate: nx ? nx.rate : null,  // rate after the next tier-up
