...
// Cloudflare's dashboard shows the S3 API URL with the bucket already appended
// (…r2.cloudflarestorage.com/droprate-games) next to a copy button, so that value
// lands in R2_ENDPOINT by accident and every signature then covers a doubled path.
// Normalise defensively rather than fail with an opaque 403.
function endpoint() {
  let e = (process.env.R2_ENDPOINT || "").trim();
  if (!e) return `https://${env("R2_ACCOUNT_ID")}.r2.cloudflarestorage.com`;
  if (!/^https?:\/\//i.test(e)) e = "https://" + e;
  e = e.replace(/\/+$/, "");
  const bucket = (process.env.R2_BUCKET || "").trim();
  if (bucket && e.toLowerCase().endsWith("/" + bucket.toLowerCase())) e = e.slice(0, -(bucket.length + 1));
  // A public bucket URL (pub-*.r2.dev or a custom domain) is READ-only — it will
  // never accept a signed PUT, and rejects it with no CORS headers, which surfaces
  // in the browser as a bare "network error". That value belongs in R2_PUBLIC_BASE,
  // so if it turns up here, ignore it rather than sign an unusable URL.
  let host = "";
  try { host = new URL(e).host.toLowerCase(); } catch { host = ""; }
  const isApiEndpoint = host.endsWith(".r2.cloudflarestorage.com");
  if (!isApiEndpoint && process.env.R2_ACCOUNT_ID) {
