...
function playUrlFor(p) {
  ...
  const base = gateBase();
  if (base && key && process.env.R2_GATE_SECRET) {
    const t = playToken(p.id);
    return t ? `${base}/t/${t}/${key.split("/").map(encodeURIComponent).join("/")}` : null;
  }
  // no gate configured yet — fall back to the plain public URL
