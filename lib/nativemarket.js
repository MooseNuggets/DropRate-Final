...
const ROYALTY_MAX_BPS = 2500;   // 25% cap on resale royalty
const ROYALTY_DEFAULT_BPS = 2500; // 25% — the headline split is 70 seller / 25 dev / 5 platform
...
/* Does this viewer get to play? Two ways in, and they are deliberately separate:

     demo_public  — the developer opened the game up for anyone. No wallet, no
                    purchase, no chain call. Costs nothing to check.
     ownership    — this wallet still holds a copy, checked against the chain,
                    not against our own records. A copy sold ten minutes ago
                    fails here even though our row still names the old buyer.

   That second point is the whole design. Nobody revokes access on resale; the
   play token simply expires within two hours and the next one is refused. */
async function mayPlay(p, wallet) {
  if (p.demo_public) return { may: true, why: "demo" };
  if (!wallet) return { may: false, why: "no wallet connected" };
  try {
    const { walletOwnsCopy } = await import("./nativebuy.js");
    const r = await walletOwnsCopy(p.id, wallet);
    return r.owns ? { may: true, why: "owns", asset: r.asset } : { may: false, why: "this wallet doesn't own a copy" };
  } catch (e) {
    // Never let a chain hiccup silently hand out a play link.
    return { may: false, why: "could not verify ownership right now" };
  }
}
...
//
// Runs ONCE per warm serverless instance, not once per request. The DDL below is
// ~19 statements that every native call paid for before doing any actual work —
// pointless catalog round-trips that helped keep the database from ever scaling
// to zero, which is what exhausted the compute quota. The promise is memoised so
// concurrent requests share one run, and cleared on failure so a transient error
// can't poison the instance for good.
let nativeMigration = null;
function migrateNative() {
  if (!nativeMigration) {
    nativeMigration = runMigrateNative().catch((e) => { nativeMigration = null; throw e; });
  }
  return nativeMigration;
}

async function runMigrateNative() {
  /* dev_sellers lives in devmarket.js, and the table below has a foreign key into
     it while several ALTERs further down add columns to it. devmarket() dispatches
     native-* actions to this module BEFORE it runs its own migration, so on a
     fresh, empty database the very first request the store makes (native-list)
     would land here with dev_sellers not yet created and the CREATE would fail.
     Harmless on the live database where those tables already exist; fatal on a
     new one. Dynamic import keeps the two modules free of a load-time cycle. */
  const { migrate: migrateDev } = await import("./devmarket.js");
  await migrateDev();
  ...
  /* Pre-owned listings. The storefront feed reads this table for its
     "pre-owned from $X" badge, so it has to exist before the FIRST catalog
     read — not only after the first purchase. (nativebuy.js declares the same
     table idempotently; whichever runs first wins.) */
  ...
  /* One-time backfill. These gates are new; anything that already existed was
     ...
     because this still runs once per cold start — an unguarded UPDATE here
...
export async function nativemarket(req, res) {
  ...
  try {
    ...
    /* ---- BUYING lives in its own module ---------------------------------
       Purchases, minting and ownership are a different concern from the
       catalog, and big enough to deserve their own file. Imported lazily so a
       plain storefront read never pays for loading the chain libraries. */
    if (typeof b.action === "string" && (b.action.startsWith("native-buy-") || b.action === "native-owns")) {
      const { nativebuy } = await import("./nativebuy.js");
      return await nativebuy(req, res, b.action, b);
    }

    /* Developer earnings and withdrawals, likewise its own module. Money leaving
       the treasury deserves to be readable in one file. */
    if (typeof b.action === "string" && b.action.startsWith("native-payout-")) {
      const { nativepayout } = await import("./nativepayout.js");
      return await nativepayout(req, res, b.action, b);
    }

    /* The desktop launcher: pairing to a wallet, and downloading what you own.
       Separate because it authenticates with a DEVICE TOKEN rather than a wallet
       signature — a desktop app has no browser extension to sign with. */
    if (typeof b.action === "string" && b.action.startsWith("native-device-")) {
      const { devicepair } = await import("./devicepair.js");
      return await devicepair(req, res, b.action, b);
    }

    /* Pre-owned resale: escrowed player-to-player sales with the dev's royalty. */
    if (typeof b.action === "string" && b.action.startsWith("native-resale-")) {
      const { nativeresale } = await import("./nativeresale.js");
      return await nativeresale(req, res, b.action, b);
    }
    ...
    if (b.action === "native-market") {
      const rows = await sql`
        SELECT p.*, s.wallet AS seller_wallet, s.studio, s.avatar, s.standing,
               (SELECT count(*)::int FROM dev_native_listings l WHERE l.product_id = p.id AND l.status = 'active') AS preowned_count,
               (SELECT min(price_cents) FROM dev_native_listings l WHERE l.product_id = p.id AND l.status = 'active') AS preowned_from_cents
        FROM dev_native_products p JOIN dev_sellers s ON s.id = p.seller_id
      ...
      const games = rows.rows
        // a sold-out finite run stays on the shelf while pre-owned copies are listed
        .filter((p) => !(p.supply_model === "finite" && availOf(p) <= 0 && !(p.preowned_count > 0)))
        .map((p) => ({
          ...publicProduct(p, { wallet: p.seller_wallet, studio: p.studio, avatar: p.avatar, standing: p.standing }),
          preowned_count: Number(p.preowned_count || 0),
          preowned_from_cents: p.preowned_from_cents == null ? null : Number(p.preowned_from_cents),
        }));
    ...
    if (b.action === "native-view") {
      ...
      const product = publicProduct(p, { wallet: p.seller_wallet, studio: p.studio, avatar: p.avatar, standing: p.standing });

      /* A play link for an owner. The wallet must be SIGNED — otherwise anyone
         could name a wallet that owns a copy and be handed a token for it.
         Unsigned callers still get the page, just without the link. */
      if (b.wallet && b.signature && (p.web_bundle_key || p.web_bundle_url)) {
        try {
          const wallet = assertSig(b);
          const verdict = await mayPlay(p, wallet);
          product.may_play = verdict.may;
          product.play_reason = verdict.why;
          if (verdict.may) product.play_url = playUrlFor(p);
          if (verdict.asset) product.owned_asset = verdict.asset;
        } catch (e) {
          product.may_play = false;
          product.play_reason = e.status === 401 ? "wallet signature not valid" : "could not check ownership";
        }
      }
