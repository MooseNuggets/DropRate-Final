...
/* Games sell for USDC or SOL, full stop. $DROP is never a store currency, so it
   is never something a developer is owed here. */
const CCY = ["USDC", "SOL"];
const SIG_TTL = Number(process.env.DEV_SIG_TTL_SEC || 300);
const decimalsFor = (c) => (c === "SOL" ? SOL_DECIMALS : USDC_DECIMALS);
...
async function sendPayout(currency, wallet, amountRaw) {
  if (!CCY.includes(currency)) throw new Error(`nativepayout: unsupported currency ${currency}`);
  return sendTreasuryMulti(wallet, amountRaw, currency);
}
...
export async function nativepayout(req, res, action, b) {
  ...
  if (action === "native-payout-request") {
    ...
    if (!currency) return res.status(400).json({ error: "pick USDC or SOL" });
