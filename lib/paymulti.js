...
export async function buildDirectPaymentMulti(payer, reference, toWallet, amountRaw, currency) {
  const { PublicKey, Transaction, SystemProgram, ComputeBudgetProgram } = await w3();
  const connection = await getConnection();
  ...
  /* Priority fee — see the long note in lib/solana.js. Without one, a loaded
     validator drops the transaction and the wallet reports a timeout rather than
     a failure. USDC has the same two-instruction shape as $DROP and fails the
     same way; SOL is light enough that it was landing without one, which is why
     only $DROP looked broken. */
  tx.add(ComputeBudgetProgram.setComputeUnitLimit({
    units: currency === "SOL" ? 20_000 : 120_000,
  }));
  tx.add(ComputeBudgetProgram.setComputeUnitPrice({
    microLamports: Number(process.env.SOLANA_PRIORITY_MICROLAMPORTS || 20000),
  }));
...
export async function findDirectPayment(reference, currency) {
  ...
  for (const ix of [...top, ...inner]) {
    ...
    if (type === "transferChecked") {
      ...
      // NEVER info.source: that is the token ACCOUNT, not the wallet that owns
      // it. See the long note on extractLegs in lib/solana.js — this exact
      // fallback addressed mints to the buyer's own ATA.
      sender = sender || info.authority || null;
...
// ---- CRATE SPLIT (SOL/USDC): one buyer-signed tx, three legs -----------------
// Crates paid in SOL or USDC settle 70 / 15 / 15 → TREASURY_WALLET /
// MARKETING_WALLET / OWNER_WALLET, no burn (nothing to burn — it isn't $DROP).
// `legs` = { treasuryRaw, marketingRaw, ownerRaw } in base units (from
// splitPaymentFiat in lib/payment.js). The reference key is attached to the
// treasury leg only; findSplitPaymentMulti returns every leg in the tx.
export function splitWalletsFiat() {
  return {
    treasury: requireEnv("TREASURY_WALLET"),
    marketing: requireEnv("MARKETING_WALLET"),
    owner: requireEnv("OWNER_WALLET"),
  };
}

export async function buildSplitPaymentMulti(payer, reference, legs, currency) {
  const { PublicKey, Transaction, SystemProgram, ComputeBudgetProgram } = await w3();
  const connection = await getConnection();
  const payerPk = new PublicKey(payer);
  const refPk = new PublicKey(reference);
  const wallets = splitWalletsFiat();
  const plan = [
    ["treasury", legs.treasuryRaw],
    ["marketing", legs.marketingRaw],
    ["owner", legs.ownerRaw],
  ].filter(([, raw]) => BigInt(raw ?? 0) > 0n);
  if (!plan.length) throw new Error("paymulti: split has no non-zero legs");

  const tx = new Transaction();
  tx.add(ComputeBudgetProgram.setComputeUnitLimit({
    units: currency === "SOL" ? 40_000 : 260_000, // 3 transfers (+3 idempotent ATA creates for USDC)
  }));
  tx.add(ComputeBudgetProgram.setComputeUnitPrice({
    microLamports: Number(process.env.SOLANA_PRIORITY_MICROLAMPORTS || 20000),
  }));
  ...
  if (currency === "SOL") {
    plan.forEach(([who, raw], i) => {
      const ix = SystemProgram.transfer({
        fromPubkey: payerPk, toPubkey: new PublicKey(wallets[who]), lamports: BigInt(raw),
      });
      if (i === 0) ix.keys.push({ pubkey: refPk, isSigner: false, isWritable: false });
      tx.add(ix);
    });
  } else if (currency === "USDC") {
    const {
    ...
    const { programId, decimals } = await mintContext(connection, mintPk);
    const fromAta = await getAssociatedTokenAddress(mintPk, payerPk, true, programId);
    for (const [i, [who, raw]] of plan.entries()) {
      const toPk = new PublicKey(wallets[who]);
      const toAta = await getAssociatedTokenAddress(mintPk, toPk, true, programId);
      tx.add(createAssociatedTokenAccountIdempotentInstruction(payerPk, toAta, toPk, mintPk, programId));
      const ix = createTransferCheckedInstruction(fromAta, mintPk, toAta, payerPk, BigInt(raw), decimals, [], programId);
      if (i === 0) ix.keys.push({ pubkey: refPk, isSigner: false, isWritable: false });
      tx.add(ix);
    }
  } else {
...
// Same finder as findDirectPayment — it already returns EVERY transfer leg in
// the tx, so a 3-leg split comes back as three legs. Exposed under its own name
// so crate.js reads clearly; destinations are wallets (SOL) or ATAs (USDC).
export async function findSplitPaymentMulti(reference, currency) {
  return findDirectPayment(reference, currency);
}

// Resolve the three split destinations to what a verifier will see on-chain:
// wallet addresses for SOL, USDC ATAs for USDC.
export async function resolveSplitDestinationsFiat(currency) {
  const wallets = splitWalletsFiat();
  if (currency === "SOL") return wallets;
  const out = {};
  for (const [k, v] of Object.entries(wallets)) out[k] = await resolveUsdcAta(v);
  return out;
}
...
export async function sendTreasuryMulti(toWallet, amountRaw, currency) {
  const { PublicKey, Transaction, SystemProgram, sendAndConfirmTransaction, ComputeBudgetProgram } = await w3();
  const connection = await getConnection();
