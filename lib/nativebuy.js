...
import {
  ...
  ownerOf, walletStillOwns, setAssetUri, MINT_COST_SOL, authorityAddress,
} from "./chain.js";
import { writeCollectionMeta, writeCopyMeta } from "./gamemeta.js";

/* Games are bought in SOL or USDC. Not $DROP.

   $DROP is deliberately NOT a payment option here, and holding it earns no
   discount on a game. The token lives in the raffles and the crates; a game
   purchase is a clean transaction in a stable-ish asset, so a buyer never has
   to think about a volatile token to buy a $15 game, and a developer never has
   to think about it to know what they'll earn. */
const CCY = ["USDC", "SOL"];
...
/* One flat platform fee on every game sale, whichever currency it's paid in.
   5% — a sixth of Steam. Kept as a single number on purpose: two rates is how a
   storefront ends up promising devs one figure and paying them another. */
const NATIVE_FEE_BPS = Number(process.env.NATIVE_FEE_BPS ?? process.env.DEV_FEE_BPS ?? 500);
const feeBpsFor = () => NATIVE_FEE_BPS;

const httpErr = (code, msg) => { const e = new Error(msg); e.status = code; throw e; };
const ccyDecimals = (cur) => (cur === "SOL" ? SOL_DECIMALS : USDC_DECIMALS);

function quoteRaw(usd, cur, prices) {
  const unit = cur === "USDC" ? 1 : prices.sol;
  if (!unit) return null;
...
async function migrateBuy() {
  ...
  /* Who holds the copy NOW, as far as DropRate's own resale market knows. Updated
     on mint and on every in-platform resale; the chain is still the final word
     (walletStillOwns), this just tells us WHICH assets to ask the chain about. */
  await sql`ALTER TABLE dev_native_copies ADD COLUMN IF NOT EXISTS current_owner text`;
  await sql`UPDATE dev_native_copies SET current_owner = first_owner WHERE current_owner IS NULL`;
  await sql`CREATE INDEX IF NOT EXISTS idx_ncopy_current ON dev_native_copies(current_owner)`;
  /* A resale royalty is a sale for the earnings ledger, so it lives in orders
     with kind='resale' — the payout code needs no change to include it. */
  await sql`ALTER TABLE dev_native_orders ADD COLUMN IF NOT EXISTS kind text NOT NULL DEFAULT 'sale'`;
  /* Resale listings (owned by lib/nativeresale.js; created here too so the
     library query above never races the resale module's first migration). */
  await sql`CREATE TABLE IF NOT EXISTS dev_native_listings(
    id serial PRIMARY KEY,
    product_id int NOT NULL REFERENCES dev_native_products(id),
    asset_address text NOT NULL,
    collection_address text NOT NULL,
    ...
    price_cents int NOT NULL,
    status text NOT NULL DEFAULT 'escrow_pending',   -- escrow_pending|active|sold|cancelled|expired
    escrow_sig text,
    return_sig text,
    created_at timestamptz NOT NULL DEFAULT now(),
  ...
  await sql`CREATE INDEX IF NOT EXISTS idx_nlist_product ON dev_native_listings(product_id, status)`;
  await sql`CREATE INDEX IF NOT EXISTS idx_nlist_asset ON dev_native_listings(asset_address)`;
  migrated = true;
}
...
async function ensureCollection(product) {
  ...
  /* Write the metadata BEFORE minting. A wallet that fetches the uri a second
     after the transaction lands should find a document there, not a 404 it
     caches. */
  let uri = "";
  try { uri = await writeCollectionMeta(product); }
  catch (e) { console.error("collection metadata write failed:", e.message); }
...
export async function walletOwnsCopy(productId, wallet) {
  ...
  const r = await sql`
    ...
    WHERE product_id = ${productId} AND (current_owner = ${wallet} OR first_owner = ${wallet})
    ORDER BY minted_at DESC LIMIT 12`;
  if (!r.rows.length) return { owns: false, asset: null };
  const live = await walletStillOwns(r.rows.map((x) => x.asset_address), wallet);
  if (live.owns) return live;
  // Same grace as the web library: a copy we just returned from escrow to this
  // wallet (delist) is theirs even if the RPC read hasn't caught up yet.
  const back = await sql`
    SELECT l.asset_address FROM dev_native_listings l
    WHERE l.seller = ${wallet} AND l.status = 'cancelled' AND l.return_sig IS NOT NULL
      AND l.closed_at > now() - interval '10 minutes'
      AND l.asset_address = ANY(${r.rows.map((x) => x.asset_address)})
    ORDER BY l.id DESC LIMIT 1`;
  if (back.rows.length) return { owns: true, asset: back.rows[0].asset_address, unverified: true };
  return live;
}
...
export async function nativebuy(req, res, action, b) {
  ...
  if (action === "native-buy-quote") {
    ...
    const prices = { sol: solUsd };
    // Only quote in what this developer actually accepts. A quote the checkout
    // then refuses at open time is a broken promise, not a helpful default.
    const accepted = (p.accepted_currencies || CCY).filter((c) => CCY.includes(c));
    const quotes = {};
    for (const cur of accepted) {
      const raw = quoteRaw(usd, cur, prices);
      if (raw != null && raw > 0n) {
        quotes[cur] = { amount_raw: raw.toString(), decimals: ccyDecimals(cur), usd: Number(usd.toFixed(4)) };
      }
    }
  ...
  if (action === "native-buy-open") {
    ...
    if (!CCY.includes(b.pay_currency)) {
      return res.status(400).json({ error: "games are paid for in USDC or SOL" });
    }
    ...
    const usd = p.price_cents / 100;
    const grossRaw = quoteRaw(usd, cur, { sol: solUsd });
  ...
  if (action === "native-buy-buildpay") {
    ...
    const built = await buildDirectPaymentMulti(payer, o.reference, requireTreasury(), o.pay_amount_raw, o.pay_currency);
  ...
  if (action === "native-buy-mint") {
    ...
    let copyUri = "";
    try { copyUri = await writeCopyMeta(p, o.copy_number); }
    catch (e) { console.error("copy metadata write failed:", e.message); }
  ...
  /* ---- SUBMIT: send the mint the buyer has just signed ---------------------
     Separate from building it because a wallet cannot reliably SEND a
     transaction that already carries other signatures — Phantom re-serialises
     it and can drop them, reporting only "Internal error". The browser signs
     with signTransaction (which preserves them) and posts the bytes here, and
     this sends them over the server's RPC.

     Nothing is trusted from the client beyond the bytes themselves: the chain
     rejects anything that isn't a properly signed transaction, and
     native-buy-confirm still verifies the resulting asset independently before
     a copy is recorded. */
  if (action === "native-buy-submit") {
    const r = await sql`SELECT * FROM dev_native_orders WHERE id = ${Number(b.order_id)}`;
    const o = r.rows[0];
    if (!o) return res.status(404).json({ error: "order not found" });
    if (o.status === "complete") {
    ...
    if (o.status !== "minting" && o.status !== "paid") {
      return res.status(409).json({ error: `order is ${o.status} — pay first` });
    }

    try {
      const { submitSignedTransaction } = await import("./solana.js");
      const signature = await submitSignedTransaction(b.signed_tx);
      return res.status(200).json({ ok: true, signature });
    } catch (e) {
      // Pass the chain's own words straight through. Substituting a friendlier
      // guess here is exactly what made the last failure undiagnosable.
  ...
  if (action === "native-buy-confirm") {
    ...
    try {
      await sql`
        INSERT INTO dev_native_copies(product_id, order_id, asset_address, collection_address, first_owner, current_owner, copy_number)
        VALUES (${o.product_id}, ${o.id}, ${asset}, ${p.collection_address}, ${o.buyer}, ${o.buyer}, ${o.copy_number})`;
    } catch {
  ...
  if (action === "native-buy-library") {
    ...
    const rows = await sql`
      SELECT c.asset_address, c.copy_number, c.minted_at, c.collection_address, c.current_owner AS db_owner, c.first_owner,
             p.id AS product_id, p.title, p.slug, p.image, p.runtime, p.price_cents, p.resale_floor_bps, p.resale_cooldown_hours,
             (SELECT id FROM dev_native_listings l WHERE l.asset_address = c.asset_address AND l.status IN ('escrow_pending','active') ORDER BY id DESC LIMIT 1) AS listing_id,
             (SELECT price_cents FROM dev_native_listings l WHERE l.asset_address = c.asset_address AND l.status = 'active' ORDER BY id DESC LIMIT 1) AS listing_price_cents,
             (SELECT id FROM dev_native_listings l WHERE l.asset_address = c.asset_address ORDER BY id DESC LIMIT 1) AS last_listing_id,
             (SELECT status FROM dev_native_listings l WHERE l.asset_address = c.asset_address ORDER BY id DESC LIMIT 1) AS last_listing_status,
             (SELECT seller FROM dev_native_listings l WHERE l.asset_address = c.asset_address ORDER BY id DESC LIMIT 1) AS last_listing_seller,
             (SELECT return_sig FROM dev_native_listings l WHERE l.asset_address = c.asset_address ORDER BY id DESC LIMIT 1) AS last_listing_return_sig
      FROM dev_native_copies c JOIN dev_native_products p ON p.id = c.product_id
      WHERE c.current_owner = ${wallet} OR c.first_owner = ${wallet} ORDER BY c.minted_at DESC LIMIT 100`;
    const escrow = chainConfigured() ? await authorityAddress() : null;
    const out = [];
    for (const row of rows.rows) {
      let owner = await ownerOf(row.asset_address);
      let returned_sig = null;
      // Self-heal: a copy sitting in our escrow with no open listing, whose last
      // listing was this wallet's and was cancelled, belongs back with them.
      // Escrow only ever holds copies for open listings, so this is unambiguous.
      if (owner && escrow && owner === escrow && !row.listing_id &&
          row.last_listing_status === "cancelled" && row.last_listing_seller === wallet) {
        if (row.last_listing_return_sig) {
          // The return transfer was already sent and confirmed; this RPC read is
          // just behind it. Trust the receipt rather than re-sending.
          owner = wallet;
        } else try {
          const { transferFromEscrow } = await import("./chain.js");
          returned_sig = await transferFromEscrow({ assetAddress: row.asset_address, collectionAddress: row.collection_address, toWallet: wallet });
          await sql`UPDATE dev_native_listings SET return_sig = ${returned_sig} WHERE id = ${row.last_listing_id} AND return_sig IS NULL`;
          owner = wallet;
        } catch (e) { console.error("library escrow return failed:", row.asset_address, e); owner = await ownerOf(row.asset_address); }
      }
      // A null owner means the RPC read failed, not that the copy is gone —
      // fall back to what we last recorded rather than telling them it's sold.
      const verified = owner !== null;
      const still_owned = verified ? owner === wallet : (row.db_owner || row.first_owner) === wallet;
      const { last_listing_id, last_listing_status, last_listing_seller, last_listing_return_sig, db_owner, ...pub } = row;
      out.push({ ...pub, still_owned, verified, current_owner: owner, listed: !!row.listing_id, returned_sig });
    }
    return res.status(200).json({ ok: true, copies: out });
  }

  /* ---- REPAIR: give existing copies their artwork -------------------------
     Copies minted before the metadata layer show as blank cards in a wallet.
     This rewrites the JSON and points each asset at it. Admin-only: it spends
     the authority's SOL and touches assets other people own, even if all it can
     change is where the picture lives. */
  if (action === "native-buy-repair-meta") {
    ...
    if (!p.collection_address) return res.status(400).json({ error: "this game has no collection yet" });

    try { await writeCollectionMeta(p); }
    catch (e) { return res.status(500).json({ error: "collection metadata: " + e.message }); }

    const copies = await sql`
      SELECT asset_address, copy_number FROM dev_native_copies
      WHERE product_id = ${pid} ORDER BY copy_number`;

    const done = [], failed = [];
    for (const c of copies.rows) {
      try {
        const uri = await writeCopyMeta(p, c.copy_number);
        await setAssetUri({
          assetAddress: c.asset_address,
          collectionAddress: p.collection_address,
          uri,
          name: `${p.title} #${c.copy_number}`,
        });
        done.push(c.copy_number);
      } catch (e) {
        failed.push({ copy: c.copy_number, error: e.message });
      }
    }
    return res.status(200).json({ ok: true, repaired: done, failed });
  }
