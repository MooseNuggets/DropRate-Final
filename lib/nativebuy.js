...
import {
  ...
  ownerOf, walletStillOwns, setAssetUri, MINT_COST_SOL,
} from "./chain.js";
...
export async function nativebuy(req, res, action, b) {
  ...
  if (action === "native-buy-library") {
    ...
    const rows = await sql`
      SELECT c.asset_address, c.copy_number, c.minted_at, c.collection_address,
    ...
    for (const row of rows.rows) {
      ...
      out.push({ ...row, still_owned: owner === wallet, current_owner: owner, listed: !!row.listing_id });
    }
