...
/* Platform fee on a native sale. Kept as its own knob rather than inherited from
   the key marketplace: a native game is a different product with a different
   cost base, and whatever the storefront promises devs should be set here. */
const NATIVE_FEE_BPS = {
  DROP: Number(process.env.NATIVE_FEE_BPS_DROP ?? 250),
  USDC: Number(process.env.NATIVE_FEE_BPS_USDC ?? 800),
  SOL: Number(process.env.NATIVE_FEE_BPS_SOL ?? 800),
};
