...
const {
    getAssociatedTokenAddress, createAssociatedTokenAccountIdempotentInstruction, createTransferInstruction,
  } = await spl();
...
const toAta = await getAssociatedTokenAddress(mint, ownerPk, true);
    tx.add(createAssociatedTokenAccountIdempotentInstruction(payerPk, toAta, ownerPk, mint));
