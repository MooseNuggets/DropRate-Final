...
/* ---------------------------------------------------------------------------
   ...
   Every transaction built or sent from this file gets one. Without a compute
   unit price, a validator under load is free to drop the transaction, and the
   wallet then reports a *timeout* rather than a failure — which is the least
   debuggable outcome available: the payment neither succeeded nor visibly
   failed, and the buyer is left staring at a spinner.

   Token transfers are the first casualties because they burn far more compute
   than a bare SOL transfer. That is why $DROP purchases timed out while SOL
   purchases went through: the SOL path is one lightweight SystemProgram
   instruction, and everything here is two or more against the token program.

   The cost is negligible. 20,000 microLamports/CU over 120,000 CU is 2,400
   lamports — a small fraction of a cent. Raise SOLANA_PRIORITY_MICROLAMPORTS in
   the environment if the network is congested; there is a lot of headroom
   before this is worth caring about.
--------------------------------------------------------------------------- */
const PRIORITY_MICROLAMPORTS = () =>
  Number(process.env.SOLANA_PRIORITY_MICROLAMPORTS || 20000);

async function priorityIxs(units) {
  const { ComputeBudgetProgram } = await w3();
  return [
    // An explicit limit keeps the fee predictable: the price is charged per
    // compute unit REQUESTED, so leaving it at the 1.4M default would multiply
    // the fee by roughly ten for no benefit.
...
// Total $DROP held by a wallet across ALL its token accounts for the mint (raw
// base units, as a BigInt). Used for the crate holder discount. Fails soft: any
// RPC error returns 0n, so a flaky RPC costs the buyer a discount, never a mint.
export async function dropBalanceRaw(wallet) {
  try {
    const { PublicKey } = await w3();
    const connection = await getConnection();
    const mintPk = new PublicKey(requireEnv("DROP_MINT"));
    const owner = new PublicKey(wallet);
    const res = await connection.getParsedTokenAccountsByOwner(owner, { mint: mintPk }, "confirmed");
    let total = 0n;
...
/* `sender` here becomes the order's buyer, and the buyer becomes the fee payer of
   the mint. So getting it wrong does not fail loudly — it produces a mint
   addressed to an account nobody can sign for, and the wallet reports something
   useless like "Transaction cancelled".

   For a TOKEN transfer the only correct answer is `info.authority`: the wallet
   that owns the tokens. `info.source` is the token ACCOUNT — a different address
   entirely, owned by the wallet but not equal to it. Falling back to it produced
   mints addressed to the buyer's own ATA, one wrong address per currency.
   A SOL transfer is the opposite case: it has no authority, and its `source` IS
   the wallet, which is why paying in SOL never showed this.

   If there is no authority, return null and let the caller keep the buyer it
   already had. Guessing is what caused this. */
function extractLegs(parsedTx, mint) {
  ...
  for (const ix of [...top, ...inner]) {
    ...
    if (type === "transferChecked") {
      ...
      sender = sender || info.authority || null;   // never info.source — see note above
...
export async function buildSplitPaymentTx(payer, reference, split) {
  ...
  // Three transfers, three idempotent ATA creations and a burn — by far the
  // heaviest transaction here, and the one most likely to be dropped without a fee.
  tx.add(...(await priorityIxs(300_000)));
...
export async function buildDirectPaymentTx(payer, reference, toWallet, amountRaw) {
  ...
  const tx = new Transaction();
  tx.add(...(await priorityIxs(120_000)));
...
/* Send a transaction the BUYER has just finished signing.

   The mint leaves the server already signed by the new asset's keypair and by
   the collection authority, with only the fee payer's slot left open. Phantom's
   signAndSendTransaction re-serialises whatever it is handed and can discard
   those existing signatures, which it then reports as a bare "Internal error" —
   no program log, no reason, nothing to act on. So the browser signs with
   signTransaction, which preserves them, and posts the finished bytes here.

   Payments never hit this because the buyer is their only signer. That is the
   whole reason paying worked while minting did not.

   Sending server-side is better regardless: it uses the configured RPC instead
   of whatever the browser can reach, and the signature is known here the moment
   it lands. */
export async function submitSignedTransaction(base64Tx) {
  const connection = await getConnection();
  const raw = Buffer.from(String(base64Tx || ""), "base64");
  if (!raw.length) throw new Error("no transaction to submit");

  // skipPreflight stays off: a malformed or failing transaction should be
  // rejected here, with the chain's reason, rather than vanishing into the void.
...
export async function sendTreasuryTransfer(toWallet, amountRaw) {
  ...
  const tx = new Transaction();
  // A dropped payout is worse than a dropped purchase: a developer is owed money
  // and the send silently fails to confirm.
  tx.add(...(await priorityIxs(120_000)));
