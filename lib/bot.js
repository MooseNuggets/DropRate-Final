...
export function formatNextDrop(draw, stock = {}, now = Date.now()) {
  ...
  if (typeof stock.holderTickets === "number") {
    const holder = stock.holderTickets;
    const free = typeof stock.freeTickets === "number" ? stock.freeTickets : 0;
    const total = holder + free;
    lines.push(`🎫 Pool: <b>${fmt(total)}</b> tickets (${fmt(holder)} holder + ${fmt(free)} free)`);
  }
