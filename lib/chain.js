...
export async function buildMintTransaction({ collectionAddress, buyerWallet, name, uri, attributes }) {
  ...
  /* Pin the transaction version explicitly. umi's default has changed before, and
     the client has to pick VersionedTransaction.deserialize vs Transaction.from —
     it should be told, not left to sniff bytes. `versioned: true` goes back with
     the payload so the browser and the launcher both decode it the same way. */
  const built = await builder.setFeePayer(buyer).setBlockhash(latest).useV0().build(umi);
  ...
  return {
    ...
    versioned: true,                          // decode with VersionedTransaction
...
// ---------------------------------------------------------------------------
// REPAIR — point an existing copy at its metadata.
//
// Copies minted before the metadata layer existed carry an empty uri, so wallets
// render them as blank cards. DropRate holds the collection's update authority,
// so it can set the uri after the fact. This is the ONLY thing that authority is
// used for besides authorising mints — it cannot move, freeze, or take a copy.
// ---------------------------------------------------------------------------
export async function setAssetUri({ assetAddress, collectionAddress, uri, name }) {
  const umi = await getUmi();
  const { publicKey } = await umiCore();
  const { update, fetchAsset, fetchCollection } = await mplCore();
...
// ---------------------------------------------------------------------------
// TRANSFER — moving a copy between wallets, for the resale market.
//
// Two shapes. buildTransferTransaction is for a copy the PLAYER holds: it comes
// back unsigned with the owner as fee payer, the browser or launcher gets the
// owner to sign it, and the server submits the bytes (the same sign-don't-send
// pattern as minting). transferFromEscrow is for a copy DropRate's authority
// holds in escrow: the authority signs and sends it itself. The authority can
// only ever move what it holds — it has no power over a copy in a player's
// wallet, so "yours to keep" stays literally true.
// ---------------------------------------------------------------------------
export async function buildTransferTransaction({ assetAddress, collectionAddress, fromWallet, toWallet }) {
  const umi = await getUmi();
  const { publicKey, createNoopSigner } = await umiCore();
  const { transferV1 } = await mplCore();
  const owner = createNoopSigner(publicKey(fromWallet));
  ...
  const latest = await umi.rpc.getLatestBlockhash();
  const built = await builder.setFeePayer(owner).setBlockhash(latest).useV0().build(umi);
  return {
...
export async function transferFromEscrow({ assetAddress, collectionAddress, toWallet }) {
  const umi = await getUmi();
  const { publicKey } = await umiCore();
  const { transferV1 } = await mplCore();
  const r = await transferV1(umi, {
    asset: publicKey(assetAddress),
    collection: collectionAddress ? publicKey(collectionAddress) : undefined,
    newOwner: publicKey(toWallet),
